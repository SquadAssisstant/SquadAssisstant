...
export async function GET(req: Request) {
  ...
  if (!isDroneKind(up.data.kind)) {
    return NextResponse.json(
      { ok: false, error: `Unsupported kind "${String(up.data.kind)}" for drone components details` },
      { status: 400 }
    );
  }
  ...
  if (!facts) {
    const legacyKey = `${s.profileId}:drone:components`;
