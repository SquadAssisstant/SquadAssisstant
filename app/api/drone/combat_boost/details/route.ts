...
export async function GET(req: Request) {
  ...
  if (!isDroneKind(up.data.kind)) {
    return NextResponse.json(
      { ok: false, error: `Unsupported kind "${String(up.data.kind)}" for drone combat boost details` },
      { status: 400 }
    );
  }
  ...
  let facts = byUpload.data ?? null;

  if (!facts) {
    const legacyKey = `${s.profileId}:drone:combat_boost`;
