...
export async function GET(req: Request) {
  ...
  if (!isHeroKind(up.data.kind)) {
    return NextResponse.json(
      { ok: false, error: `Unsupported kind "${String(up.data.kind)}" for hero skills details` },
      { status: 400 }
    );
  }
  ...
  if (!facts) {
    const legacyKey = `${s.profileId}:hero_skills:upload_${uploadId}`;
